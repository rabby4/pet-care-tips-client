...
"use client" // Ensure this component is client-only
...
const RichForm = () => {
	const [isClient, setIsClient] = useState(false) // Track client-only code
	const { user } = useUser() // Retrieve user from context (ensure `user.provider` has "use client")
	const [value, setValue] = useState("")
	const [imageFile, setImageFiles] = useState<File | undefined>()
	const [imagePreviews, setImagePreviews] = useState<string | null>()
	const [selectedValue, setSelectedValue] = useState("")
	const { mutate: handleCreatePost } = useCreatePost() // Ensure `useCreatePost` is client-only
	const [isPremium, setIsPremium] = useState(false)
	const { isOpen, onOpen, onOpenChange } = useDisclosure()
	...
	// Only render client-only components if `isClient` is true
	if (!isClient) {
		return null
	}
					...
					<Button onPress={onOpen} className="bg-transparent p-0">
						<Avatar
							...
							src={
								user?.image
									? user.image
									: "https://i.ibb.co.com/H7zTvh7/user.png" // Placeholder image for logged-out users
							}
						/>
						<Input
							placeholder={
								user
									...
									: "Log in to share your thoughts" // Placeholder for logged-out users
							}
						/>
					</Button>
					<Modal
													...
													<div className="mb-5">
														...
														{!user?.premium && (
															<>
																...
																<p className="text-sm">
																	To become a premium member, click{" "}
																	<Link
																		className="text-primary underline"
																		href={"/pricing"}
																	>
